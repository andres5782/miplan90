...
const DEFAULT_PLAN = [
  {D:"d2",C:"c1",N:"n1"},{D:"d1",C:"c2",N:"n3"},{D:"d3",C:"c3",N:"n2"},
  {D:"d4",C:"c4",N:"n6"},{D:"d1",C:"c5",N:"n4"},{D:"d2",C:"c6",N:"n5"},{D:"d4",C:"c1",N:"n6"},
];
const SHOPPING = {
...
export default function App() {
  ...
  const nextT   = TRAININGS[(lastTId?(lastTId%4):0)];
  ...
  const proteinCounts = useMemo(()=>{
    const c={}; plan.forEach(day=>{ ["C","N"].forEach(meal=>{ const m=MEALS[meal].find(x=>x.id===day[meal]); const p=m?.protein; if(p&&p!=="libre"&&p!=="otro"&&p!=="huevos") c[p]=(c[p]||0)+1; }); }); return c;
  },[plan]);

  const tomorrowMeals = useMemo(()=>{
    const idx=(todayIdx+1)%7; return ["C","N"].map(meal=>{const m=MEALS[meal].find(x=>x.id===plan[idx][meal]);return m?.freeze?m:null;}).filter(Boolean);
  },[plan,todayIdx]);
  ...
  // UI helpers — all colors from T, nothing hardcoded
    ...
    <div style={{minHeight:"100vh",background:T.bg2,color:T.text,fontFamily:"-apple-system,'Helvetica Neue',sans-serif",maxWidth:500,margin:"0 auto"}}>
      ...
      {section==="home" && (<>
        ...
        <div style={{padding:"14px 14px 90px"}}>
          ...
          {/* Hoy toca */}
          <div style={{...card({padding:16,marginBottom:10,background:`linear-gradient(135deg,${nextT.color}10,${T.bg})`,border:`1px solid ${nextT.color}28`})}}>
            <div style={{fontSize:11,fontWeight:700,color:nextT.color,letterSpacing:0.3,marginBottom:8}}>HOY TOCA</div>
          ...
          <div style={{...card({padding:16,marginBottom:10})}}>
            <div style={{fontSize:11,fontWeight:700,color:T.green,letterSpacing:0.3,marginBottom:12}}>HOY COMES</div>
            {["D","C","N"].map(meal=>{
              const icons={D:"🌅",C:"☀️",N:"🌙"};
              const labels={D:"Desayuno",C:"Comida",N:"Cena"};
              const m=MEALS[meal].find(x=>x.id===todayMeals?.[meal]);
              ...
              return (
                <div key={meal} style={{display:"flex",alignItems:"center",gap:10,paddingBottom:meal!=="N"?12:0,marginBottom:meal!=="N"?12:0,borderBottom:meal!=="N"?`1px solid ${T.bg3}`:"none"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:13,fontWeight:700,color:isLibre?T.purple:T.text}}>{m?.label||"—"}</div>
                    <div style={{fontSize:11,color:T.text3,marginTop:1}}>{m?.desc}</div>
                  </div>
                  <button onClick={()=>{setSection("food");setFoodTab("week");setOpenDay(todayIdx);}} style={{background:T.bg2,border:`1px solid ${T.bg3}`,borderRadius:8,padding:"6px 9px",cursor:"pointer",fontSize:13,color:T.text3}}>✏️</button>
                </div>
              );
            })}
          </div>
          ...
          {tomorrowMeals.length>0 && (
            <div style={{background:"#f0f6ff",border:`1px solid ${T.acc}25`,borderRadius:14,padding:"12px 14px",marginBottom:10}}>
              <div style={{fontSize:11,fontWeight:700,color:T.acc,letterSpacing:0.3,marginBottom:6}}>❄️ DESCONGELA ESTA NOCHE</div>
                ...
                <div key={di} style={{...card({marginBottom:8})}}>
                  <button onClick={()=>setOpenDay(isOpen?null:di)} style={{width:"100%",background:"none",border:"none",padding:"12px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer",color:"inherit"}}>
                    ...
                    <div style={{flex:1,display:"flex",gap:5}}>
                      {["D","C","N"].map(meal=>{
                        const m=MEALS[meal].find(x=>x.id===plan[di][meal]);
                        return (
                          <div key={meal} style={{flex:1,background:T.bg2,borderRadius:7,padding:"5px 6px",fontSize:9,color:m?.protein==="libre"?T.purple:T.text2,lineHeight:1.3}}>
                            ...
                            {!isEditing ? (
                              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                                <div style={{flex:1,background:T.bg2,borderRadius:10,padding:"9px 11px"}}>
                                  <div style={{fontSize:13,fontWeight:600,color:m?.protein==="libre"?T.purple:T.text}}>{m?.label}</div>
        ...
        <div style={{background:T.bg,borderBottom:`1px solid ${T.bg3}`,padding:"56px 20px 0",position:"sticky",top:0,zIndex:50}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:4}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:nextT.color,fontWeight:700}}>Hoy toca: {nextT.label} {nextT.emoji}</div>
            </div>
